...
class PagerPageHolder(
    ...
    private val translation = PageTranslationBinder(
        ...
        pageKey = { "${page.chapter.chapter.id}:${page.index}" },
        pageBytes = {
            page.stream?.let { open -> open().use { process(item, Buffer().readFrom(it)) }.readByteArray() }
        },
    )
    ...
    /** Translates a user-selected area. [viewRect] is in this view's space. */
    fun translateRegion(viewRect: android.graphics.RectF) = translation.translateRegion(viewRect)
